...
excerpt: "CVE-2022-26134 is a 0-day RCE in Confluence. We are scanning the internet for exposed servers and warning owners. If you host Confluence the advice is to apply the patch and if that is not possible to take it offline"
...
- Other versions of Atlassian Confluence are not yet tested and are potentially vulnerable as well
versions:
- ">= 7.4.0"
- Older versions are potentially vulnerable too.
recommendation: Patches are available. If you can not patch, we recommended to restrict access to Confluence or to take it offline.
patch_status: Patch available
workaround: If access cannot be blocked or Confluencec cannot be taken offline a WAF that blocks requests containing `${` may provide some protection
status: Open
start: 2022-06-03
end:
timeline:
